...
export const Completed: RequestHandler = {
    canHandle(handlerInput) {
        return IsIntentWithCompleteDialog(handlerInput, IntentTypes.ProphecyPriceCheck);
    },
    async handle(handlerInput) {
        ...
        const item = slots[SlotTypes.Prophecy];
        ...
        if (item && item.isMatch && !item.isAmbiguous) {
            ...
            try {
                const [, res] = await Promise.all([
                    ...
                    apiClient.items({
                        league: league,
                        type: ItemRequestTypes.Prophecy,
                        date: CurrentDate(),
                    }),
                ]);
                ...
                throw CreateError(`Got error while getting prophecy prices: ${err}`, ErrorTypes.API)
            }
        }

        throw CreateError(`Got to the COMPLETED state of ${IntentTypes.ProphecyPriceCheck} without a slot.`, ErrorTypes.Unexpected);
    }
};
