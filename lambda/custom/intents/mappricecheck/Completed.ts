...
export const Completed: RequestHandler = {
    canHandle(handlerInput) {
        return IsIntentWithCompleteDialog(handlerInput, IntentTypes.MapPriceCheck);
    },
    async handle(handlerInput) {
        ...
        const item = slots[SlotTypes.Map];
        ...
        if (item && item.isMatch && !item.isAmbiguous) {
            ...
            try {
                ...
                throw CreateError(`Got error while getting map prices: ${err}`, ErrorTypes.API)
            }
        }

        throw CreateError(`Got to the COMPLETED state of ${IntentTypes.MapPriceCheck} without a slot.`, ErrorTypes.Unexpected);
    }
};
