...
export const Completed: RequestHandler = {
    canHandle(handlerInput) {
        return IsIntentWithCompleteDialog(handlerInput, IntentTypes.UniqueJewelPriceCheck);
    },
    async handle(handlerInput) {
        ...
        const item = slots[SlotTypes.UniqueJewel];
        ...
        if (item && item.isMatch && !item.isAmbiguous) {
            ...
            try {
                const [, res] = await Promise.all([
                    ...
                    apiClient.items({
                        league: league,
                        type: ItemRequestTypes.UniqueJewel,
                        date: CurrentDate(),
                    }),
                ]);
                ...
                throw CreateError(`Got error while getting unique jewel prices: ${err}`, ErrorTypes.API)
            }
        }

        throw CreateError(`Got to the COMPLETED state of ${IntentTypes.UniqueJewelPriceCheck} without a slot.`, ErrorTypes.Unexpected);
    }
};
