...
export const Completed: RequestHandler = {
    canHandle(handlerInput) {
        return IsIntentWithCompleteDialog(handlerInput, IntentTypes.DivinationPriceCheck);
    },
    async handle(handlerInput) {
        ...
        const item = slots[SlotTypes.Divination];
        ...
        if (item && item.isMatch && !item.isAmbiguous) {
            ...
            try {
                const [, res] = await Promise.all([
                    ...
                    apiClient.items({
                        league: league,
                        type: ItemRequestTypes.DivinationCard,
                        date: CurrentDate(),
                    }),
                ]);
                ...
                throw CreateError(`Got error while getting divination prices: ${err}`, ErrorTypes.API)
            }
        }

        throw CreateError(`Got to the COMPLETED state of ${IntentTypes.DivinationPriceCheck} without a slot.`, ErrorTypes.Unexpected);
    }
};
