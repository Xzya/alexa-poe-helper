...
export const Completed: RequestHandler = {
    canHandle(handlerInput) {
        return IsIntentWithCompleteDialog(handlerInput, IntentTypes.EssencePriceCheck);
    },
    async handle(handlerInput) {
        ...
        const item = slots[SlotTypes.Essence];
        ...
        if (item && item.isMatch && !item.isAmbiguous) {
            ...
            try {
                ...
                throw CreateError(`Got error while getting essence prices: ${err}`, ErrorTypes.API)
            }
        }

        throw CreateError(`Got to the COMPLETED state of ${IntentTypes.EssencePriceCheck} without a slot.`, ErrorTypes.Unexpected);
    }
};
