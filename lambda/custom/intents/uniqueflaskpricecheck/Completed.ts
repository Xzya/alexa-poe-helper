...
export const Completed: RequestHandler = {
    canHandle(handlerInput) {
        return IsIntentWithCompleteDialog(handlerInput, IntentTypes.UniqueFlaskPriceCheck);
    },
    async handle(handlerInput) {
        ...
        const item = slots[SlotTypes.UniqueFlask];
        ...
        if (item && item.isMatch && !item.isAmbiguous) {
            ...
            try {
                const [, res] = await Promise.all([
                    ...
                    apiClient.items({
                        league: league,
                        type: ItemRequestTypes.UniqueFlask,
                        date: CurrentDate(),
                    }),
                ]);
                ...
                throw CreateError(`Got error while getting unique flask prices: ${err}`, ErrorTypes.API)
            }
        }

        throw CreateError(`Got to the COMPLETED state of ${IntentTypes.UniqueFlaskPriceCheck} without a slot.`, ErrorTypes.Unexpected);
    }
};
