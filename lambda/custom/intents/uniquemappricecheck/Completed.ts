...
export const Completed: RequestHandler = {
    canHandle(handlerInput) {
        return IsIntentWithCompleteDialog(handlerInput, IntentTypes.UniqueMapPriceCheck);
    },
    async handle(handlerInput) {
        ...
        const item = slots[SlotTypes.UniqueMap];
        ...
        if (item && item.isMatch && !item.isAmbiguous) {
            ...
            try {
                const [, res] = await Promise.all([
                    ...
                    apiClient.items({
                        league: league,
                        type: ItemRequestTypes.UniqueMap,
                        date: CurrentDate(),
                    }),
                ]);
                ...
                throw CreateError(`Got error while getting unique map prices: ${err}`, ErrorTypes.API)
            }
        }

        throw CreateError(`Got to the COMPLETED state of ${IntentTypes.UniqueMapPriceCheck} without a slot.`, ErrorTypes.Unexpected);
    }
};
